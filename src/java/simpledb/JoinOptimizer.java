...
public class JoinOptimizer {
     ...
     */
    public JoinOptimizer(final LogicalPlan p, final Vector<LogicalJoinNode> joins) {
        this.p = p;
        this.joins = joins;
    }
     ...
     */
    public static OpIterator instantiateJoin(final LogicalJoinNode lj,
                                             final OpIterator plan1, final OpIterator plan2) throws ParsingException {
        ...
        final JoinPredicate p = new JoinPredicate(t1id, lj.p, t2id);
     ...
     */
    public static int estimateTableJoinCardinality(final Predicate.Op joinOp,
                                                   final String table1Alias, final String table2Alias, final String field1PureName,
                                                   final String field2PureName, final int card1, final int card2, final boolean t1pkey,
                                                   final boolean t2pkey, final Map<String, TableStats> stats,
                                                   final Map<String, Integer> tableAliasToId) {
        int card = 1;
        if (joinOp.equals(Predicate.Op.EQUALS)) {
            if (t1pkey || t2pkey) {
                card = t1pkey && t2pkey ? Math.min(card1, card2) : t1pkey ? card2 : card1;
            } else {
                card = Math.max(card1, card2);
            }
        } else {
            return (int) (card1 * card2 * 0.7);
        }
     ...
     * cost2
     */
    public double estimateJoinCost(final LogicalJoinNode j, final int card1, final int card2,
                                   final double cost1, final double cost2) {
     ...
     */
    public int estimateJoinCardinality(final LogicalJoinNode j, final int card1, final int card2,
                                       final boolean t1pkey, final boolean t2pkey, final Map<String, TableStats> stats) {
        if (j instanceof LogicalSubplanJoinNode) {
            // A LogicalSubplanJoinNode represents a subquery.
            // You do not need to implement proper support for these for Lab 3.
            return card1;
    ...
    public <T> Set<Set<T>> enumerateSubsets(final Vector<T> v, final int size) {
        ...
        for (int i = 0; i < size; i++) {
            final Set<Set<T>> newels = new HashSet<Set<T>>();
            for (Set<T> s : els) {
                for (T t : v) {
                    final Set<T> news = (Set<T>) (((HashSet<T>) s).clone());
                    if (news.add(t)) {
                        newels.add(news);
                    }
                }
    ...
    public Vector<LogicalJoinNode> orderJoins(
            final HashMap<String, TableStats> stats,
            final HashMap<String, Double> filterSelectivities, final boolean explain)
            throws ParsingException {
        final PlanCache planCache = new PlanCache();
        for (int i = 1; i <= joins.size(); i++) {
            final Set<Set<LogicalJoinNode>> subsets = enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> set : subsets) {
                CostCard bestPlan = null;
                double bestCost = Double.MAX_VALUE;
                for (LogicalJoinNode s : set) {
                    bestPlan = computeCostAndCardOfSubplan(stats, filterSelectivities, s, set, bestCost, planCache);
                    if (bestPlan != null) {
                        bestCost = bestPlan.cost;
    ...
    private CostCard computeCostAndCardOfSubplan(
            final HashMap<String, TableStats> stats,
            final HashMap<String, Double> filterSelectivities,
            final LogicalJoinNode joinToRemove, final Set<LogicalJoinNode> joinSet,
            final double bestCostSoFar, final PlanCache pc) throws ParsingException {

        LogicalJoinNode j = joinToRemove;

        final Vector<LogicalJoinNode> prevBest;
        ...
        final String table1Name = Database.getCatalog().getTableName(
                this.p.getTableId(j.t1Alias));
        final String table2Name = Database.getCatalog().getTableName(
                this.p.getTableId(j.t2Alias));
        final String table1Alias = j.t1Alias;
        final String table2Alias = j.t2Alias;

        final Set<LogicalJoinNode> news = (Set<LogicalJoinNode>) ((HashSet<LogicalJoinNode>) joinSet)
                .clone();
        news.remove(j);
        ...
        if (news.isEmpty()) { // base case -- both are base relations
            ...
            final double prevBestCost = pc.getCost(news);
            final int bestCard = pc.getCard(news);
        ...
        final LogicalJoinNode j2 = j.swapInnerOuter();
        final double cost2 = estimateJoinCost(j2, t2card, t1card, t2cost, t1cost);
        if (cost2 < cost1) {
            final boolean tmp;
            j = j2;
            cost1 = cost2;
        ...
        final CostCard cc = new CostCard();
     ...
     * otherwise
     */
    private boolean doesJoin(final Vector<LogicalJoinNode> joinlist, final String table) {
     ...
     */
    private boolean isPkey(final String tableAlias, final String field) {
        final int tid1 = p.getTableId(tableAlias);
        final String pkey1 = Database.getCatalog().getPrimaryKey(tid1);
     ...
     * joinlist
     */
    private boolean hasPkey(final Vector<LogicalJoinNode> joinlist) {
        for (LogicalJoinNode j : joinlist) {
     ...
     *                      alias is given)
     */
    private void printJoins(final Vector<LogicalJoinNode> js, final PlanCache pc,
                            final HashMap<String, TableStats> stats,
                            final HashMap<String, Double> selectivities) {
        ...
        final HashMap<String, DefaultMutableTreeNode> m = new HashMap<String, DefaultMutableTreeNode>();

        // int numTabs = 0;

        // int k;
        DefaultMutableTreeNode root = null, treetop = null;
        final HashSet<LogicalJoinNode> pathSoFar = new HashSet<LogicalJoinNode>();
        boolean neither;
        ...
        final JTree tree = new JTree(treetop);
        final JScrollPane treeView = new JScrollPane(tree);

        tree.setShowsRootHandles(true);

        // Set the icon for leaf nodes.
        final ImageIcon leafIcon = new ImageIcon("join.jpg");
